rem  SPDX-License-Identifier: GPL-2.0
rem 
rem  Nuvoton IGPS: Image Generation And Programming Scripts For Arbel BMC
rem 
rem  Copyright (C) 2022 Nuvoton Technologies, All Rights Reserved
rem -------------------------------------------------------------------------

"C:\Program Files (x86)\DediProg\SF100\dpcmd.exe" -u .\py_scripts\ImageGeneration\output_binaries\Secure\Kmt_TipFwL0_Skmt_TipFwL1_BootBlock_BL31_Tee_uboot.bin --type W25Q256JV

