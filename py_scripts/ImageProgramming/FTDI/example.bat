...
:: open by description and issue power cycle with FUP mode enable . 
Arbel_EVB_Util -open-desc "NPCM8mnx_Evaluation_Board A" %POWER_OFF% %STRAP9_ASSERTED% -update -delay 3000 %POWER_ON% -update -delay 1000

:loop

:: if more then one Arbel EVB is connected, use -open-sn or -open-locid command to distinguish between boards. 
:: open by serial-number and issue PORST cycle with security mode enable.  
Arbel_EVB_Util -open-sn "Arbel-EVB-001-A" %PORST_ASSERTED% %STRAP8_ASSERTED% -update -delay 1000 %PORST_RELEASE% -update -delay 1000 -pin-read %PORST_N%
echo Funtion returned: %ERRORLEVEL%

Arbel_EVB_Util -open-sn "Arbel-EVB-028-A" %PORST_ASSERTED% %STRAP8_ASSERTED% -update -delay 1000 %PORST_RELEASE% -update -delay 1000 -pin-read %PORST_N%
echo Funtion returned: %ERRORLEVEL%

goto loop

:: program a new FTDI device  
:: Arbel_EVB_Util -open-desc "Quad RS232-HS A" -newdesc "NPCM8mnx_Evaluation_Board" -newmf "Nuvoton" -newsn "Arbel-EVB-001-" -prog
